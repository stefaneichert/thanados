...
function appendvis(iter) { //append vis form to dialog
        ...
        $( '#VisSelect_' + iter).on('change', function() {
            ...
            if (visappendLevel !== 'findcount') $( '#myvisform').append(
...
$('#myvisform').append(
    ...
    $( '#opacityvalue').on('input change', function() {
               ...
               if (myopacity > 100) $( '#opacityvalue').val(100);
               if (myopacity < 0) $( '#opacityvalue').val(0);
    ...
    $( '#borderwidth').on('input change', function() {
               ...
               if (myborderwidth < 0) $( '#borderwidth').val(0);
    });
...
getChoroplethJson(visproperty, visappendLevel, mylegendtitle, mysteps, mymode, [colorstart, colorend], mybordercolor, myborderwidth, myfinalopacity, mylegend)
}
...
function getChoroplethJson (visproperty, visappendLevel, title, mysteps, mymode, mycolor, myborder, myborderwidth, myfinalopacity, mylegend) {
   mychorojson = {"type": "FeatureCollection", //prepare geojson
                 "features": []
                };
//todo quantify age, sex,
...
setChoropleth(title, mysteps, mymode, mycolor, myborder, myborderwidth, myfinalopacity, mylegend);
}
...
if (typeof(legend) !== 'undefined') $('.legendtoremove').remove();
