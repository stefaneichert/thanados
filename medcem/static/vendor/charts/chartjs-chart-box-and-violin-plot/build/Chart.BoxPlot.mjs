import { defaults, Element, canvasHelpers, elements, helpers, controllers, scaleService, LinearScaleBase } from 'chart.js';
...
var defaults$1 = _objectSpread({}, defaults.global.elements.rectangle, {
  borderWidth: 1,
  outlierRadius: 2,
  outlierColor: defaults.global.elements.rectangle.backgroundColor,
  medianColor: null,
  itemRadius: 0,
  itemStyle: 'circle',
  itemBackgroundColor: defaults.global.elements.rectangle.backgroundColor,
  itemBorderColor: defaults.global.elements.rectangle.borderColor,
  hitPadding: 2,
  tooltipDecimals: 2
});
...
defaults.global.elements.boxandwhiskers = _objectSpread({}, defaults$1);
...
var BoxAndWiskers = elements.BoxAndWhiskers = ArrayElementBase.extend({
  transition: function transition(ease) {
    ...
    if (model.boxplot === view.boxplot) {
      view.boxplot = helpers.clone(view.boxplot);
    }
...
defaults.global.elements.violin = _objectSpread({
  points: 100
}, defaults$1);
...
var Violin = elements.Violin = ArrayElementBase.extend({
  transition: function transition(ease) {
    ...
    if (model.violin === view.violin) {
      view.violin = helpers.clone(view.violin);
    }
...
defaults.boxplot = helpers.merge({}, [defaults.bar, verticalDefaults, defaults$2]);
defaults.horizontalBoxplot = helpers.merge({}, [defaults.horizontalBar, horizontalDefaults, defaults$2]);
...
defaults.violin = helpers.merge({}, [defaults.bar, verticalDefaults, defaults$3]);
defaults.horizontalViolin = helpers.merge({}, [defaults.horizontalBar, horizontalDefaults, defaults$3]);
...
var ArrayLinearScaleOptions = helpers$1.merge({}, [commonScaleOptions, scaleService.getScaleDefaults('linear')]);
var ArrayLinearScale = scaleService.getScaleConstructor('linear').extend({
  getRightValue: function getRightValue$$1(rawValue) {
    return LinearScaleBase.prototype.getRightValue.call(this, getRightValue(rawValue));
  },
...
var helpers$2 = helpers;
var ArrayLogarithmicScaleOptions = helpers$2.merge({}, [commonScaleOptions, scaleService.getScaleDefaults('logarithmic')]);
...
export { BoxAndWiskers as BoxAndWhiskers, Violin, ArrayLinearScale, ArrayLogarithmicScale, BoxPlot, HorizontalBoxPlot, HorizontalViolin };
