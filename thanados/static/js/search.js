...
function appendSearch() {//append search form to dialog
    ...
    $('#LevelSelect_' + Iter).on('change', function () {
        ...
        $('#Form' + Iter).append(
            ...
            (checkAvailable(appendLevel, 'value') ? '<option title="Classifications of entities that are connected with values (e.g. maximum age, body height etc.)" value="value">Value property</option>' : '') +
            '</select>\n' +
            '</div>'
        );
...
//define basemaps
    ...
    eval('natural' + Iter + ' = jQuery.extend(true, {}, basemap_cz);');
    eval('terrain' + Iter + ' = jQuery.extend(true, {}, Esri_WorldHillshadeNoOp);');
    ...
    eval('MyBaseLayers' + Iter + ' = {"Landscape": landscape' + Iter + ', "Satellite": satellite' + Iter + ', "Streetmap": streets' + Iter + ', "Basemap": natural' + Iter + ', "Terrain": terrain' + Iter + ', "Blank": blank' + Iter + '};');
