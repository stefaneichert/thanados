...
def edm(img_id=None):
    ...
    def get_lan_labels(id):
        sql = """
            ...
            WHERE ab.domain_id IN (197088,197086) AND a.id =%(id)s"""
    ...
    for row in data['subject_str']:
        for key, item in row.items():
            tag = 'dc:subject'
            create_subelement(provided_cho, tag, item, key)

    for url in data.get('subject_rdf', []):
    ...
    # - dcterms:spatial
    for row in data['spatial_str']:
        for key, item in row.items():
            tag = 'dcterms:spatial'
            create_subelement(provided_cho, tag, item, key)

    for spatial_entry in data.get('spatial', []):
