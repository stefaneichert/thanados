...
|| (SELECT count(child_id)::TEXT FROM devill.EntCount WHERE openatlas_class_name = 'artifact' 
        AND site_id IN %(site_ids)s AND Path LIKE 'Artifact >%%') || ','
|| (SELECT count(child_id)::TEXT FROM devill.EntCount WHERE openatlas_class_name = 'human_remains' 
